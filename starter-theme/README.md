...
A generic, ready-to-brand theme built on the Octet-SCSS core (app shell, nav, footer, base document styles, typography, block spacing, utilities, icon helpers). It's meant to be **copied out of the framework** and customized, so you can pull framework updates without your changes ever conflicting.

## Quick start

**1. Copy this folder out of the submodule** into your project, next to `octet-scss` — e.g. `src/my-theme/`:
...
**3. Point your app at it.** Import `src/my-theme/main.scss` wherever you load styles (e.g. a layout component), instead of `octet-scss/main.scss`.

**4. Brand it.** Copy `_theme-colors.scss` to `_brand.scss` and override the ramps / semantic tokens in `:root`. No Sass required — it's all custom properties:
...
- **Name classes with BEM** — block · `block__element` · `block--modifier`, state on attributes, IDs for hooks not styling. See [Naming conventions](https://claude.ai/README.md#naming-conventions) in the core README; the shipped chrome here (`.nav`, `.footer`, `.app-shell`) are worked examples.
